...
SELECT avg(GRADES.G_GRADE),STUDENTS.ST_ID_NUM,ST_ID,ST_NAME FROM GRADES
INNER JOIN STUDENTS ON GRADES.G_STUDENT = STUDENTS.ST_ID
...
GROUP BY STUDENTS.ST_ID_NUM, ST_ID, ST_NAME ;

/*Delete any grade from the table GRADES, by specifying student and course*/
DELETE FROM GRADES
WHERE GRADES.G_STUDENT =
      (SELECT STUDENTS.ST_ID FROM STUDENTS WHERE ST_NAME='Janis')
AND GRADES.G_COURSE =
    (SELECT COURSES.C_ID FROM COURSES WHERE C_TITLE='Advanced Data Technologies')
;

/*Change students ID number of any student*/
UPDATE STUDENTS
SET STUDENTS.ST_ID_NUM='UPDATED' WHERE ST_ID_NUM='071RDB161';

/*Add one column to any table*/
ALTER TABLE STUDENTS ADD ST_GENDER VARCHAR2(1) DEFAULT 'M' NULL;